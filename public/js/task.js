$(document).ready(function(){
    ...
    const $formAddComment = $('#addCommentaireForm');
    const interval = 250;
    ...
    $cardFooter.on('click', '.add-task', function(){
        ...
        $.ajax({
            ...
            data: {
                ...
                'statut': $formAdd.find('#task-status').val(),
            ...
            success: function(data) {
                if((data.errors)){
                    ...
                    $formAdd.modal('hide');
                    toastr.success('Task Ajouté!', 'success', {timeOut: timeoutDefault});
    ...
    $cardFooter.on('click', '.edit-task-save', function(){
        ...
        $.ajax({
            ...
            data:{
                ...
                'statut': $formEdit.find('#task-status').val(),
            ...
            success: function(data) {
                if((data.errors)){
                    ...
                    $formEdit.modal('hide');
                    toastr.success('Task Modifié!', 'Success', {timeOut: timeoutDefault});
                    $('.card-'+data.task.id).remove();
                    $('.' +data.task.statut).prepend(data.card);
                }
            },
        });
    });
    ...
    $cardFooter.on('click', '.delete-task-confirm', function(){
        ...
        $.ajax({
            ...
            success: function(data) {
              ...
              toastr.success('Task Supprimé!','Success', {timeOut: timeoutDefault})
              $('.card-'+data.id).remove();
            }
        });
    });
    ...
    $cardFooter.on('click', '.add-comment', function() {
        ...
        $.ajax({
            ...
            type: "POST",
            url: 'commentaires/add/'+ id,
            data:{
                '_token': $formAddComment.find('input[name=_token]').val(),
                'commentaire': $formAddComment.find('#task-comment').val(),
            ...
            success: function(data) {
                if((data.errors)){
                    ...
                    toastr.success('Commentaire ajouté!', 'Success', {timeOut: timeoutDefault});
                    getComments(id);
                }
              },
        });
    ...
    function getComments(taskId){
        $.ajax({
            url:'commentaires/'+ taskId,
            success: function(data) {
                $('#task-comments').html(data.commentaire);
            },
